...
class RsaServiceTests {
    ...
    decryptInvalidArguments() {
        ...
        unit.exception(_ => {
            unit.when('Invalid source encoding', this._rsa.decrypt({ key: PRIVATE_KEY, input: ENCRYPT_STRING, source_encoding: <any>98765 }));
        }).isInstanceOf(Error).hasProperty('message', 'Invalid source encoding');
        unit.exception(_ => {
            unit.when('Non allowed source encoding', this._rsa.decrypt({ key: PRIVATE_KEY, input: ENCRYPT_STRING, source_encoding: 'xaxaxa' }));
        }).isInstanceOf(Error).hasProperty('message', 'Source encoding must be one of the following: "utf8,base64,hex,binary", Provided: "xaxaxa"');
    }
}
